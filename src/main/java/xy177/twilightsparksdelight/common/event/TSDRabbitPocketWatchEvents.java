...
/**
 * Addon behavior around Twilight Forest's native pocket watch.
 *
 * The watch item itself is intentionally supplied by Twilight Forest.
 */
public final class TSDRabbitPocketWatchEvents {
    ...
    private static final TagKey<net.minecraft.world.item.Item> KNIFE_TAG = TagKey.create(
    ...
    public static void onEffectApplicable(MobEffectEvent.Applicable event) {
        if (event.getEntity() instanceof Player player
                && event.getEffectInstance().is(MobEffects.DIG_SLOWDOWN)
    ...
    public static void onIncomingDamage(LivingIncomingDamageEvent event) {
        ...
        TSDTriggers.GUNFIRE_BREAKS_RABBIT_WATCH.get().trigger(shooter);
    }
    ...
    private static boolean hasPocketWatchInHand(Player player) {
        return player.getMainHandItem().is(TFItems.POCKET_WATCH.get())
                || player.getOffhandItem().is(TFItems.POCKET_WATCH.get())
    ...
    private static boolean hasPocketWatchInHotbar(Player player) {
        for (int slot = 0; slot < 9; slot++) {
            if (player.getInventory().getItem(slot).is(TFItems.POCKET_WATCH.get())) {
                return true;
            }
        }
        return player.getOffhandItem().is(TFItems.POCKET_WATCH.get())
                || xy177.twilightsparksdelight.integration.CuriosCompat.hasWatch(player);
    }

    private static void giveQuietEffect(Player player, net.minecraft.core.Holder<net.minecraft.world.effect.MobEffect> effect) {
