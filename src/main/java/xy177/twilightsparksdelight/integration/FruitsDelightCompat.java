...
public final class FruitsDelightCompat {
    ...
    public static Result process(TSDGloryCrucibleBlockEntity crucible, ItemStack stack, boolean simulate) {
        ...
        var interaction = interactions.get(stack.getItem());
        ...
        if (!simulate) {
            ...
            if (serving) {
                ...
                if (crucible.getVolumeUnits() > 0) {
                    crucible.setFruitState(state.hasProperty(LayeredCauldronBlock.LEVEL)
                            ? state.setValue(LayeredCauldronBlock.LEVEL, 3) : state);
                }
            } else {
