...
public final class CopperCupCompat {
    private static final ResourceLocation CUP =
            ResourceLocation.fromNamespaceAndPath("minersdelight", "copper_cup");
    ...
    public static java.util.List<RecipeHolder<CookingPotRecipe>> recipeViews(Level level, ItemStack cup) {
        if (cup.isEmpty()) return java.util.List.of();
        var result = new java.util.ArrayList<RecipeHolder<CookingPotRecipe>>();
        for (var holder : level.getRecipeManager().getAllRecipesFor(
            ...
            var view = new CookingPotRecipe(original.getGroup(), original.getRecipeBookTab(),
                    original.getIngredients(), serving, cup.copyWithCount(2),
                    original.getExperience(), original.getCookTime());
            result.add(new RecipeHolder<>(TwilightSparksDelight.id(
                    holder.id().getNamespace() + "/" + holder.id().getPath() + "_copper_cup_jei"), view));
        }
