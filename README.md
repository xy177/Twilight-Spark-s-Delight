...
**Twilight Spark's Delight** is an original Farmer's Delight addon built around the world of The Twilight Forest. This workspace contains its **Minecraft 1.21.1 NeoForge edition**.
...
Farmer's Delight and The Twilight Forest are required. Twilight Forest's native Pocket Watch and Mason Jar are reused rather than registered a second time.
...
Use **Java 21** and the included Gradle wrapper. Fixed development dependencies
are downloaded automatically from NeoForge, Modrinth and CurseMaven; third-party
...
Output: `build/libs/twilight_spark_delight-1.21.1-1.0.0.jar`.

`build.gradle` lists the pinned dependency versions and development run options.
Optional local JAR overrides may be placed in the ignored `libs/` directory;
`-PremoteDependencies` bypasses those overrides. `-PwithoutOptionalMods` leaves
optional integrations out of the development runtime, not the compiled mod.

This branch contains production sources and build files, not the local migration
test worlds, diagnostics or screenshots. It is not an automatic converter for
Forge 1.12.2 worlds.
